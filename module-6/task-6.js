// 1
const listRows = document.getElementById('list')
const items = listRows.getElementsByTagName('li')
...
buttonChange.addEventListener('click', showElements)
...
function changeCss() {
    ...
    paragraphs[0].innerText = `First paragraph`
...
const showImages = document.querySelectorAll('.shoe')
const showGradients = document.querySelectorAll('.gradient')
...
Array.from(colors).forEach(color => {
    color.addEventListener('click', () => {
        ...
        Array.from(showImages).forEach((image) => {
        ...
        Array.from(showGradients).forEach((gradient) => {
            const gradientColor = gradient.getAttribute('color')
            if (selectedColor === gradientColor) {
